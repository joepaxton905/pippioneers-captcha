...
export async function handleSignup(data) {
  try {
    ...
    await connectDB();
    const { firstname, lastname, email, phone, password, confirmPassword, captchaToken } = data;
    ...
    if (!captchaToken) {
      return { message: "Please complete the CAPTCHA verification", status: "error" };
    }
    ...
    try {
      ...
      if (!captchaVerification.data.success) {
        return { message: "CAPTCHA verification failed. Please try again.", status: "error" };
      }
    } catch (captchaError) {
